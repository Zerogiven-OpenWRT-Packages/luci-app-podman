...
const ListUtil = baseclass.extend({
    ...
    createToolbar: function (options) {
        ...
        if (options.customButtons) {
            options.customButtons.forEach((btn) => {
                buttons.push(new podmanUI.Button(btn.text, btn.handler, btn.cssClass).render());
                buttons.push(' ');
            });
        }
    ...
    bulkDelete: function (options) {
        ...
        podmanUI.showSpinningModal(_('Deleting %s').format(this.itemName + 's'), _('Deleting %d selected %s...').format(
            selected.length,
            selected.length === 1 ? this.itemName : this.itemName + 's'
        ));
        ...
        Promise.all(deletePromises).then((results) => {
            ...
            ui.hideModal();
            podmanUI.errorNotification(_('Failed to delete some %s: %s').format(this.itemName + 's', err.message));
        });
    },
    ...
    showInspect: function (identifier, hiddenFields, closeButtonFn) {
        podmanUI.showSpinningModal(_('Fetching information...'), _('Loading %s Details').format(this.itemName.charAt(0).toUpperCase() + this.itemName.slice(1)));
