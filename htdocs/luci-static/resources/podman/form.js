...
const FormContainer = baseclass.extend({
    ...
    showModal: function (images, networks) {
        ...
        field.description = _('Select network for the container. User-created networks provide better isolation and DNS resolution between containers.');
        ...
        this.map.render().then((formElement) => {
            ui.showModal('', [
                ...
                new pui.ModalButtons({
                    ...
                    onCancel: () => { ui.hideModal(); this.map.reset(); }
                }).render()
            ]);
    ...
    handleCreate: function () {
        this.map.save().then(() => {
            ...
            pui.showSpinningModal(_('Creating Container'), _('Creating container from image %s...').format(container.image));

            podmanRPC.container.create(JSON.stringify(spec)).then((result) => {
                if (result && result.error) {
                    ui.hideModal();
                    pui.errorNotification(_('Failed to create container: %s').format(result.error));
                    return;
                }

                // Check if we should start the container
                // Handle different value types from LuCI Flag component
                const shouldStart = container.start === '1' || container.start === true || container.start === 1;

                if (shouldStart && result && result.Id) {
                    // Start the container
                    pui.showSpinningModal(_('Starting Container'), _('Starting container...'));

                    podmanRPC.container.start(result.Id).then((startResult) => {
                        ui.hideModal();
                        if (startResult && startResult.error) {
                            pui.warningNotification(_('Container created but failed to start: %s').format(startResult.error));
                        } else {
                        ...
                        ui.hideModal();
                        pui.errorNotification(_('Container created but failed to start: %s').format(err.message));
                        this.submit();
                    });
                } else {
                    ui.hideModal();
                ...
                ui.hideModal();
                pui.errorNotification(_('Failed to create container: %s').format(err.message));
            });
        }).catch(() => { });
    },
            ...
            new pui.ModalButtons({
                ...
                onConfirm: () => {
                    ...
                    try {
                        ...
                        pui.errorNotification(_('Failed to parse command: %s').format(err.message));
                    }
                }
            }).render()
        ];
    ...
    createFromSpec: function (spec) {
        pui.showSpinningModal(_('Creating Container'), _('Creating container from image %s...').format(spec.image));
        ...
        podmanRPC.container.create(JSON.stringify(spec)).then((result) => {
            ...
            if (shouldStart && result && result.Id) {
                ...
                podmanRPC.container.start(result.Id).then((startResult) => {
                    ...
                    ui.hideModal();
                    pui.warningNotification(_('Container created but failed to start: %s').format(err.message));
                    this.submit();
                });
            } else {
                ui.hideModal();
...
const FormImage = baseclass.extend({
    ...
    render: function () {
        this.map = new form.JSONMap(this.data, _('Pull Image'), _('Fetch a container image using Podman.'));
    ...
    handlePullExecute: function () {
        this.map.save().then(() => {
            ...
            podmanRPC.image.pullStream(imageName).then((result) => {
                ...
                ui.hideModal();
                pui.errorNotification(_('Failed to pull image: %s').format(err.message));
            });
        });
    },
...
const FormNetwork = baseclass.extend({
    ...
    render: function () {
        ...
        field.description = _('Automatically configure OpenWrt network interface, bridge, and firewall zone. <strong>Highly recommended</strong> for proper container networking on OpenWrt.');
        ...
        field.description = _('Name of the bridge interface (e.g., podman0, mynet0). Leave empty to use: &lt;network-name&gt;0. Note: If the generated name conflicts with an existing interface, OpenWrt will auto-increment it.');
    ...
    handleCreate: function () {
        this.map.save().then(() => {
            ...
            podmanRPC.network.create(JSON.stringify(payload)).then((result) => {
                ...
                if (result && result.error) {
                    ui.hideModal();
                    pui.errorNotification(_('Failed to create network: %s').format(result.error));
                    return Promise.reject(new Error('Podman network creation failed'));
                }
                if (result && result.message && result.response >= 400) {
                    ui.hideModal();
                    pui.errorNotification(_('Failed to create network: %s').format(result.message));
                    return Promise.reject(new Error('Podman network creation failed'));
                }
                if (result && result.cause) {
                    ui.hideModal();
                    pui.errorNotification(_('Failed to create network: %s').format(result.cause));
                    return Promise.reject(new Error('Podman network creation failed'));
                }
                ...
                if (status.podmanCreated && status.openwrtCreated) {
                    pui.successTimeNotification(_('Network and OpenWrt integration created successfully'));
                } else if (status.podmanCreated && !status.openwrtCreated && status.openwrtError) {
                    pui.warningNotification(_('Network created but OpenWrt integration failed: %s. You may need to configure OpenWrt manually.').format(status.openwrtError));
                } else if (status.podmanCreated) {
                    pui.successTimeNotification(_('Network created successfully'));
                }
                ...
                if (err.message !== 'Podman network creation failed') {
                    pui.errorNotification(_('Failed to create network: %s').format(err.message));
                }
            });
        }).catch(() => { });
    },
    ...
    handleCreate: function () {
        this.map.save().then(() => {
            ...
            podmanRPC.pod.create(JSON.stringify(payload)).then((result) => {
                ui.hideModal();
                if (result && result.error) {
                    pui.errorNotification(_('Failed to create pod: %s').format(result.error));
                    return;
                }
                ...
                ui.hideModal();
                pui.errorNotification(_('Failed to create pod: %s').format(err.message));
            });
        }).catch(() => { });
    },
    ...
    handleCreate: function () {
        ...
        this.map.save().then(() => {
            ...
            podmanRPC.secret.create(secretName, secretData).then((result) => {
                ...
                // Check for various error formats
                if (result && result.error) {
                    pui.errorNotification(_('Failed to create secret: %s').format(result.error));
                    return;
                }
                if (result && result.message && result.response >= 400) {
                    pui.errorNotification(_('Failed to create secret: %s').format(result.message));
                    return;
                }
                if (result && result.cause) {
                    pui.errorNotification(_('Failed to create secret: %s').format(result.cause));
                    return;
                }
                ...
                pui.errorNotification(_('Failed to create secret: %s').format(errorMsg));
            });
        }).catch((err) => {
    ...
    handleCreate: function () {
        this.map.save().then(() => {
            ...
            podmanRPC.volume.create(JSON.stringify(payload)).then((result) => {
                ui.hideModal();
                if (result && result.error) {
                    pui.errorNotification(_('Failed to create volume: %s').format(result.error));
                    return;
                }
                ...
                ui.hideModal();
                pui.errorNotification(_('Failed to create volume: %s').format(err.message));
            });
        }).catch(() => { });
    },
...
const FormResourceEditor = baseclass.extend({
    ...
    render: function (containerId, containerData) {
        ...
        const data = {
            resources: {
                ...
                memorySwap: hostConfig.MemorySwap > 0 ? utils.formatBytes(hostConfig.MemorySwap, 0) : '',
        ...
        field.description = _('Total memory limit (memory + swap). -1 for unlimited swap. Leave empty for unlimited.');
        ...
        field.optional = true;
        field.validate = (_section_id, value) => {
            if (value && (parseInt(value) < 10 || parseInt(value) > 1000) && parseInt(value) !== 0) {
    ...
    handleUpdate: function () {
        this.map.save().then(() => {
            ...
            const memorySwap = resources.memorySwap === '-1' ? -1 : utils.parseMemory(resources.memorySwap, true);
            ...
            if (memorySwap === null && resources.memorySwap && resources.memorySwap !== '-1') {
                pui.errorNotification(_('Invalid memory swap format. Use: 512m, 1g, -1, etc.'));
                return;
            }
            ...
            if (resources.cpuLimit) {
                const period = 100000;
                updateData.cpu.quota = Math.floor(parseFloat(resources.cpuLimit) * period);
            ...
            podmanRPC.container.update(this.containerId, JSON.stringify(updateData)).then((result) => {
                ui.hideModal();
                if (result && result.error) {
                    pui.errorNotification(_('Failed to update resources: %s').format(result.error));
                } else {
                ...
                ui.hideModal();
                pui.errorNotification(_('Failed to update resources: %s').format(err.message));
            });
        }).catch(() => { });
    }
});
...
const FormNetworkConnect = baseclass.extend({
    ...
    handleConnect: function () {
        this.map.save().then(() => {
            ...
            podmanRPC.network.connect(networkData.name, JSON.stringify(params)).then((result) => {
                ui.hideModal();
                if (result && result.error) {
                    pui.errorNotification(_('Failed to connect to network: %s').format(result.error));
                } else {
                    pui.successTimeNotification(_('Connected to network successfully'));
                    if (this.onSuccess) this.onSuccess();
                }
            }).catch((err) => {
                ui.hideModal();
                pui.errorNotification(_('Failed to connect to network: %s').format(err.message));
            });
        }).catch(() => { });
    }
});
