...
return view.extend({
    ...
    handleBulkHealthCheck: function () {
        ...
        if (containersWithHealth.length === 0) {
            ui.addTimeLimitedNotification(null, E('p', _('No selected containers have health checks configured')), 3000, 'warning');
            return;
        }

        podmanUI.showSpinningModal(_('Running Health Checks'), _('Running health checks on selected containers...'));
        ...
        Promise.all(healthCheckPromises).then((results) => {
            ...
            if (errors.length > 0) {
                const errorMsg = errors.map((e) => `${e.id.substring(0, 12)}: ${e.error}`).join(', ');
                podmanUI.errorNotification(_('Failed to run health checks on some containers: %s').format(errorMsg));
            } else {
                podmanUI.successTimeNotification(_('Health checks completed successfully'));
            }
            this.refreshTable(false);
        });
    }
